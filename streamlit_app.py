"""
chinese_app.py — 國語測驗網頁（Streamlit Cloud + Firebase Realtime Database）
...
def load_records_cached(difficulty: str) -> list:
    ...
    先行過濾，避免每次都把整個 records_chinese 全部下載回來。
    需搭配 Firebase Rules 的 ".indexOn": ["difficulty"] 才有索引效能。
    """
    init_firebase()
    ref  = firebase_db.reference("records_chinese")
...
def filter_records(records: list, difficulty: str, period: str) -> list:
    ...
    for r in records:
        ...
        # 保留原始欄位，補上解析後的 score 與 datetime 物件方便後續排序/顯示
        record = dict(r)
        record["score"] = score
        record["_ts"]   = ts
